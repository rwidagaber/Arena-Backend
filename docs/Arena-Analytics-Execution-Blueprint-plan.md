...
Implement an enterprise-grade admin analytics stack in phased, non-breaking increments across API contracts, query model, DTOs, indexing, caching, and visualization backlog. Keep compatibility with existing dashboard contract while introducing a versioned analytics contract for richer sections.

**Steps**
1. Contract-first foundation
- Introduce a dedicated analytics API surface centered on one aggregated endpoint plus focused drilldown endpoints.
