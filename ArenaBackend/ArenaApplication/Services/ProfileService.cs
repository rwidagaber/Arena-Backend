...
namespace ArenaApplication.Services
{
    public class ProfileService : IProfileService
    {
        ...
        private readonly IMemberHealthRAGService _healthRAGService; // ✅ Add

        public ProfileService(
            IAuthRepository authRepository,
            UserManager<ApplicationUser> userManager,
            IStringLocalizer<ArenaLocalization> localizer,
            IMemberHealthRAGService healthRAGService) // ✅ Add
        {
            _authRepository = authRepository;
            _userManager = userManager;
            _localizer = localizer;
            _healthRAGService = healthRAGService; // ✅ Add
        }
        ...
        public async Task<Result<GetProfileDto>> UpdateProfileAsync(
            Guid userId, UpdateProfileDto dto)
        {
            ...
            if (activeSubscription == null)
                return Result<GetProfileDto>.Failure(
                    _localizer["ProfileLockedRequiresSubscription"]);
            ...
            if (dto.PreferredLanguage is not null) user.PreferredLanguage = dto.PreferredLanguage;
            ...
            // ✅ Track if health-related fields changed
            bool healthDataChanged = false;
            ...
            if (user.MemberProfile is not null)
            {
                ...
                if (dto.Goal is not null)
                {
                    ...
                    healthDataChanged = true; // ✅ Goal change affects health context
                }

                if (dto.TargetWeight is not null)
                    user.MemberProfile.TargetWeight = dto.TargetWeight;

                // ✅ Check if Injuries or HealthConditions changed (from DTO if present)
                if (dto.Injuries is not null)
                {
                    user.MemberProfile.Injuries = dto.Injuries;
                    healthDataChanged = true;
                }

                if (dto.HealthConditions is not null)
                {
                    user.MemberProfile.HealthConditions = dto.HealthConditions;
                    healthDataChanged = true;
                }

                if (dto.DietaryRestrictions is not null)
                {
                    user.MemberProfile.DietaryRestrictions = dto.DietaryRestrictions;
                    healthDataChanged = true;
                }
                ...
                // ✅ Re-sync health vectors if health data changed
                if (healthDataChanged && user.MemberProfile != null)
                {
                    try
                    {
                        await _healthRAGService.SyncProfileHealthDataAsync(user.MemberProfile.Id);
                        Console.WriteLine($"✅ Health vectors synced for {user.FirstName}");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"⚠️ Health vector sync failed: {ex.Message}");
                    }
                }
            }

            return Result<GetProfileDto>.Success(new GetProfileDto
            {
                Id = user.Id,
                MemberProfileId = user.MemberProfile?.Id ?? Guid.Empty,
