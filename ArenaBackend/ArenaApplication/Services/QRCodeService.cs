...
namespace ArenaInfrastructure.Services
{
    public class QRCodeService : IQRCodeService
    {
        ...
        public async Task<QrDto> GenerateAsync(Guid bookingId)
        {
            ...
            var bookingDay = booking.BookingDate.Date;
            var expirationTime = booking.BookingDate.Date.Add(
                booking.StartTime.Add(TimeSpan.FromHours(2)));

            if (bookingDay != now.Date)
                throw new InvalidOperationException("QR code is only available on the booking day.");
            ...
            if (existingQr != null)
            {
                if (existingQr.IsUsed || existingQr.ExpirationTime <= now)
                {
                    await _qrRepo.HardDeleteAsync(existingQr);
        ...
        public async Task<QrScanResultDto> ScanAsync(string code, Guid? scannedById)
        {
            ...
            // 3. Check if expired
            if (DateTime.Today < qr.GeneratedAt.Date)
                return new QrScanResultDto
                {
                    BookingId = qr.BookingId,
                    Message = _localizer["QRInvalid"]
            ...
            if (qr.ExpirationTime < DateTime.Now)
                return new QrScanResultDto
                {
                    IsExpired = true,
                    BookingId = qr.BookingId,
