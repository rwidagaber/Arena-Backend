...
namespace ArenaInfrastructure.Data.DataSeeding
{
    public static class DashboardDataSeeder
    {
        public static async Task SeedAsync(AppDbContext context, bool forceReseed = false)
        {
            ...
            var allPlans    = new[] { basicPlan, premiumPlan, elitePlan };
            ...
            void AddSubscription(MemberProfile mp, SubscriptionPlan plan,
                                 SubscriptionStatus status, DateTime subStart, DateTime subEnd)
            {
                var sub = new UserSubscription
                {
                    ...
                    EndDate           = subEnd,
                    Status            = status,
                    RemainingSessions = status == SubscriptionStatus.Active
                                        ? rng.Next(0, 20)
                                        : 0,
                    ReminderSent      = false,
                    CreatedAt         = subStart
            ...
            // How many members get Active subscriptions (6–12, capped by member count)
            int activeCount   = rng.Next(6, Math.Min(shuffled.Count, 12) + 1);
            ...
            foreach (var mp in shuffled)
            {
                ...
                for (int h = historyCount - 1; h >= 0; h--)
                {
                    ...
                    var pastStatus = rng.Next(10) < 2
                        ? SubscriptionStatus.Cancelled
                        : SubscriptionStatus.Expired;

                    AddSubscription(mp, allPlans[rng.Next(allPlans.Length)], pastStatus, subStart, subEnd);
            ...
            for (int i = 0; i < activeMembers.Count; i++)
            {
                ...
                AddSubscription(mp, allPlans[rng.Next(allPlans.Length)],
                                SubscriptionStatus.Active, subStart, subEnd);

                // 30% chance this member also has a second concurrent Active subscription
                // (e.g. gym access plan + personal training pack)
                if (rng.Next(10) < 3)
                {
                    ...
                    var otherPlans = allPlans.Where(p => p.Id != allPlans[rng.Next(allPlans.Length)].Id).ToArray();
                    var plan2 = otherPlans.Length > 0 ? otherPlans[rng.Next(otherPlans.Length)] : allPlans[rng.Next(allPlans.Length)];
            ...
            // Random base attendance level each run (slow vs busy gym)
            int baseAttendance = rng.Next(3, 9);

            // Random growth scenario: flat / growing / declining / volatile
            ...
            for (int daysAgo = 89; daysAgo >= 0; daysAgo--)
            {
                ...
                int count = (int)Math.Round(baseAttendance * dayWeights[dowIdx] * trendFactor * noise);
                ...
                foreach (var mp in dayMembers)
                {
                    ...
                    if (checkIn > now) checkIn = now.AddMinutes(-rng.Next(5, 30));

                    // Mostly Confirmed/Completed; small chance Cancelled (no-show)
                    var bStatus = rng.Next(10) < 2
